...
public class XCodeBuildOutputParser {
    ...
    protected void handleLine(String line) {
        ...
        if (!ignoreUnitTests) {
            ...
            if (m.matches()) {
                buildListener.message("##teamcity[testSuiteStarted name='" + escapeForTeamCity(m.group(1)) + "']");
                return;
            }

            m = END_SUITE.matcher(line);
            if (m.matches()) {
                buildListener.message("##teamcity[testSuiteFinished name='" + escapeForTeamCity(m.group(1)) + "']");
                return;
            }

            m = START_TESTCASE.matcher(line);
            if (m.matches()) {
                buildListener.message("##teamcity[testStarted name='" + escapeForTeamCity(m.group(1)) + "']");
                return;
            }

            m = END_TESTCASE.matcher(line);
            if (m.matches()) {
                buildListener.message("##teamcity[testFinished name='" + escapeForTeamCity(m.group(1)) + "' duration='" + m.group(2) + "']");
                return;
            }

            m = ERROR_TESTCASE.matcher(line);
            if (m.matches()) {
                buildListener.message("##teamcity[testFailed name='" + escapeForTeamCity(m.group(3)) + "' message='" + escapeForTeamCity(m.group(4)) + "']");
                return;
            }

            m = FAILED_TESTCASE.matcher(line);
            if (m.matches()) {
                buildListener.message("##teamcity[testFailed name='" + escapeForTeamCity(m.group(1)) + "' duration='" + escapeForTeamCity(m.group(2)) + "']");
                return;
            }
        }
